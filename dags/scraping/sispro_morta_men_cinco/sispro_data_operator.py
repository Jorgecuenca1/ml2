...
class DataSispro4Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind8-P')
        try:
            self.logger.info("Starting scraping sispro_morta_men_cinco")
...
class DataSispro5Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        data = PrepareData(Config.FOLDER_FILES)
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind8-D')
        try:
            self.logger.info("Starting scraping sispro_morta_men_cinco")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
...
class DataSispro7Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        minio = MinioClient()
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind8-M')
        try:
            self.logger.info("Starting scraping sispro_morta_men_cinco")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
            ...
            data.source8(f"{params_f[0]['object_locator']}_P.csv", f"{params_f[0]['object_locator']}_D.csv",
                         f"{params_f[0]['object_locator']}_M.csv")
