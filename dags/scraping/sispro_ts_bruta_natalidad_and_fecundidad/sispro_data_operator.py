...
class DataSispro4Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind5-P')
        try:
            self.logger.info("Starting scraping sispro_ts_bruta_natalidad_and_fecundidad")
...
class DataSispro5Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        data = PrepareData(Config.FOLDER_FILES)
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind5-D')
        try:
            self.logger.info("Starting scraping sispro_ts_bruta_natalidad_and_fecundidad")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
...
class DataSispro7Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        data = PrepareData(Config.FOLDER_FILES)
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind5-M')
        try:
            self.logger.info("Starting scraping sispro_ts_bruta_natalidad_and_fecundidad")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
            ...
            data.source5and6(f"{params_f[0]['object_locator']}_P.csv", f"{params_f[0]['object_locator']}_D.csv",
                         f"{params_f[0]['object_locator']}_M.csv")
