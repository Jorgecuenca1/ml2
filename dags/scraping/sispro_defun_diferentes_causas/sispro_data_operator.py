...
class DataSispro4Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind11-P')
        try:
            self.logger.info("Starting scraping sispro_defun_diferentes_causas")
            ...
            data.rename_file(f"{values[0][values[0].rfind('/') + 1:]}.csv",
                             f"{params_f[0]['object_locator']}_P.csv")
...
class DataSispro5Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        data = PrepareData(Config.FOLDER_FILES)
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind11-D')
        try:
            self.logger.info("Starting scraping sispro_defun_diferentes_causas")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
...
class DataSispro7Operator(BaseOperator):
    ...
    def execute(self, context):
        ...
        minio = MinioClient()
        params = file.get_params(Config.PARAMETERS_API_URL, token)
        params_f, xpaths, values, clicks = file.filters_params_sispro(params, 'Ind11-M')
        try:
            self.logger.info("Starting scraping sispro_defun_diferentes_causas")
            sispro4 = sispro(params_f[0]['site']['url'], Config.SELENIUM_REMOTE)
            sispro4.browser(xpaths, values)
            ...
            data.source11_14(f"{params_f[0]['object_locator']}_P.csv", f"{params_f[0]['object_locator']}_D.csv",
                         f"{params_f[0]['object_locator']}_M.csv")
            data.cast_int(['Textbox27', 'Textbox30', 'Textbox3', 'Textbox31', 'Textbox15', 'Textbox33', 'Textbox4', 'Textbox24'])
